...
class PersonReIdentifier:
    ...
    """
    def __init__(self, model_name='osnet_ain_x0_5', device='cuda' if torch.cuda.is_available() else 'cpu'):
        self.device = device
        self.model_name = model_name
        self.local_weights = "osnet_ain_x0_5_imagenet.pyth"
        ...
        num_classes=1000,
        loss="softmax",
        pretrained=False,   # triggers ImageNet weights download
        ).to("cuda").eval()


    
        '''
        # 1. Try Loading Local SOTA Model (OSNet)
        if 'osnet' in model_name and os.path.exists(self.local_weights):
            try:
                print(f"📂 Found local weights: {self.local_weights}")
                print(f"⬇️  Loading architecture for: {model_name}...")
                
                # Load architecture from Hub (needs internet/cache for definition), but don't download weights
                self.model = torch.hub.load('KaiyangZhou/deep-person-reid', model_name, pretrained=False)
                
                # Load local weights
                print(f"⚖️  Loading state dictionary...")
                # weights_only=False is required for older checkpoints containing numpy scalars
                try:
                    state_dict = torch.load(self.local_weights, map_location=self.device, weights_only=False)
                except TypeError:
                ...
                # Handle common checkpoint dict wrapping
                ...
                if missing:
                    print(f"ℹ️ Note: some keys were missing (expected for customized heads): {len(missing)}")
                
                self.model.to(self.device)
                self.model.eval()
                self.success = True
                print(f"✅ SOTA ReID Model ({model_name}) loaded from local file")
            except Exception as e:
                print(f"⚠️ Failed to load local OSNet ({e}). Fallbacks will be attempted...")

        # 2. Try SOTA ReID Model (OSNet) via Torch Hub (Auto-Download)
        if not self.success and 'osnet' in model_name:
            try:
                print(f"⬇️  Attempting to load specialized ReID model: {model_name} from Hub...")
        ...
        # 3. Fallback / Default to ResNet50 (Standard Computer Vision)
        if not self.success:
            try:
...
class BicycleSeatFactoryMonitoringSystem:
    def __init__(self, config_path="bicycle_seat_config.json", db_path="bicycle_seat_factory_monitoring.db"):
        ...
        # Uses 'osnet_ain_x0_5' to match local weights if available
        self.reid_system = PersonReIdentifier(model_name='osnet_ain_x0_5') 
